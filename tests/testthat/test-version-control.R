test_that("version control works as expected", {
  ...
  #check data merge with real data and check diff looks right
    ...
    #add a new file
    proj2 <- load_project(csv_path = test_path("testdata", "example-csv-data2.csv"), csv_files = paste0("example-csv-data", 2, ".csv"),
                         tz = "Etc/GMT+8", site="FAL", prj_path=temp_rds, username="Smith")

    expect_true(nrow(proj2$data) > nrow(proj1$data)) #data gets added
    expect_equal(names(proj2$diffs), c("dd1", "dd2")) #create two diff (one for merged data, one for filling gaps)
    diff <- proj2$diffs
    expect_equal(diff[[1]]$fDOM_QSU_flag, NULL)
    expect_true(is.data.frame(diff[[1]]$fDOM_QSU))
    expect_true(all(diff[[1]]$fDOM_QSU$op_type == "data_merge"))
    expect_equal(nrow(diff[[1]]$DateTime) + nrow(diff[[2]]$DateTime), nrow(proj2$data) - nrow(proj1$data)) #make sure that all the row changes are logged including filling a row
    ...
    #check that we could return to proj1 if we wanted
    proj1_test <- apply_diff(proj2$data, proj2$diffs, invert=TRUE, skip_merge=FALSE)
    expect_equal(proj1_test, proj1$data)
