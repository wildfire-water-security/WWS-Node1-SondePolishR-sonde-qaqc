...
test_that("data summarizing works", {
  data <- SondePolishR:::combine_flags(example_sondeproj) #this is input to summarize data
  ...
  #test 1 hour summary
    ...
    sum_data <- summarize_data(data, freq, "mean")
      ...
      #check flags
      test <- data %>% mutate(DateTime_rd = round_date(.data$DateTime_rd, freq)) %>% left_join(sum_data, by = join_by(DateTime_rd))
      ...
      #check values
      test <- data %>% mutate(DateTime_rd = round_date(.data$DateTime_rd, freq)) %>% group_by(DateTime_rd)
