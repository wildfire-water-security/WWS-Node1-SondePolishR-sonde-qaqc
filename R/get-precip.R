...
get_precip <- function(data, lat, long, method, token=NULL){
  ...
  if(method == "nldas"){
    precip <- get_nldas(token = token, lat=lat, long=long, start=dates$min, end=dates$max) %>%
      mutate(DateTime = as.POSIXct(.data$DateTime, tz = "UTC"))
  }
...
#'
#' Downloads precipitation data from [NLDAS](https://ldas.gsfc.nasa.gov/nldas). Available from NASA Earthdata.
#' This dataset is available across CONUS at a resolution of 0.125 × 0.125 degrees available from 1981 to near real time.
#' This data requires a token to access the data. See [here](https://urs.earthdata.nasa.gov/documentation/for_users/user_token) for
#' directions on creating a token. Note that this token should be kept secret.
#'
#' @param token Character of token
#' @param lat the latitude to get precipitation data at
#' @param long the longitude to get precipitation data at
#' @param start Start date to get precipitation data (should be a date or datetime)
#' @param end End date to get precipitation data (should be a date or datetime)
#'
#' @returns a data.frame with two columns:
...
#' Access your stored token via: `Sys.getenv("EARTHDATA_TOKEN")`
#'
#' @examples
