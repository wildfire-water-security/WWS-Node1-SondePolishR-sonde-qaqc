...
The app consists of nine modules or pages. *While they are put in an
...
4.  **Physical Limits:** Removes data points outside a specified range.
    Limits default to common sensor measurement limits.

5.  **Outlier Removal:** Remove outlier data points and mark
    questionable points. Removal can be done via a combination of
    automatic outlier detection and manual addition or removal of
    points.

6.  **Interpolation:** Fill in missing data points using interpolation.
...
7.  **Corrections:** Perform a shift correction for a selected set of
    points, apply a linear drift correction to a specific file, or apply
    a smoothing correction to a region. Drift corrections are based on
    calibration checks when provided.

8.  **fDOM Corrections:** Apply temperature and turbidity corrections to
    fDOM data using one of several methods. Methods default to published
    coefficient values but can be set to custom values.

9.  **Download Data:** Generate and view time summarized data; export
...
- Before loading the data, supply an analyst name, this could be your
  initials or name. It will be used in the changelog to track who made
  what change.

- This step uses the `load_project` function to read in data and format
  as a `sondeproj`. Use once you’re specified the file paths to all
...
Next to the plot you can also control the minimum and maximum y-values
to display. It will default to the maximum value of the full dataset.
You can also remove the value to have it autoscale to the data actively
being viewed.
...
This step is used to remove “bad” or erroneous data points. You can also
mark regions or points that are questionable or bad. Points marked as
bad are removed from the dataset while points marked as questionable are
flagged but retained. There are several methods that can be used to
select points for removal:
...
- **High variability:** Locates periods that have high variability
  relative to the surrounding data which may indicate unrealistic noise
...
This step is used to apply shifts to sections of data. There are three
types of corrections that can be applied. To switch between the methods,
change the selection under **Select Type**.

#### 7.1 Additive

This method is meant to correct groups of points that are shifted due to
instrument issues (i.e., sensor interference). It works similarly to [5
...
This method is used to correct for highly “messy” regions of data. The
method works by applying a smoothing correction to a selected data range
using the selection tool. Smoothing method include:
...
This method should be used sparingly as it will “rewrite” or adjust
large portions of data, including points you may have more confidence
in.

For all methods, and similar to the other modules, when you want to save
a change use the **Flag points** section. To add an optional note and
...
- **Summary Method:** Available if **export frequency** is larger than
  ...
  data. You may select more than one and they will show up as separate
  column in the exported data.
