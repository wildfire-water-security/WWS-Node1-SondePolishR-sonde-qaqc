...
3.  Log in and authorize the application by clicking **Applications** in
    the top toolbar then **Authorized Apps.** At the bottom of the page
    click **Approve more applications**, find NASA GESDISC DATA ARCHIVE,
    and click **Authorize.** Read and agree to the terms and click
    agree.

4.  Generated your token by selecting **Generate Token**. Click the
    green **Generate Token** button at the bottom of the page. Copy the
    code.

5.  Securely save your newly generated and authorized token by running
    the following code:

    `{r} usethis::edit_r_environ(scope = "user")}`

6.  Paste the text: `EARTHDATA_TOKEN = "TOKEN HERE"`, putting your token
    between the quotes.
...
> **Note:** This token will expire every few months and you will need to
> repeat steps 4 through 7 whenever the token expires.
