...
![Flow chart describing the workflow steps for the SondePolishR app.](figures/SondePolishR%20Workflow.png)
...
4.  In the **Load Data** section enter the **analyst name** you want associated with the changes. This will be stored in the changelog and should be something you're okay with being visible to anyone you share the project with.

5.  **Load the data.** Use the **Load Data** section to load your selected data by clicking the **Load Sonde Data** button.
...
6.  **Load Precipitation Data.** Precipitation data can be extremely helpful in making decisions about data corrections as it can help determine if observed spikes could have been caused by increased flows. There are currently three options for adding precipitation data to the project:
...
2.  **Remove known out of water periods.** Out of water periods are times the sonde has been removed from the stream to download data or perform maintenance. We want to remove the regions as we know that those measurements will not be representative of the stream conditions.
    ...
    -   To remove data from the highlighted periods, click the **Flag OOW Periods** button. This will remove measurements across all parameters during those periods.
        ...
        > **Note:** The measurement taken right before and after the sonde was removed from the water is also removed to prevent issues with time difference between the recorded time and the sonde time and short equilibrium periods.
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/data-checks.gif")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/turb-limits.gif")
```
...
1.  **Use a filter to auto-select likely bad points.** Under **select starting method.** Typically the **Hampel Filter** method should work for most datasets. See `identify_outliers()` for more details about selection methods.
...
2.  **Adjust the plot to better visualize the data.** See the section below, [5.2 Parameter Specific Considerations], for parameter specific considerations and recommendations.
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/turb-set-view.gif")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/turb-check-bad.gif")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/turb-flagging.gif")
```
...
-   **Additive:** Used to apply a linear or absolute shift to a set of selected data. To apply this kind of correction, ensure you're on the *Additive* option within the **Corrections** panel.
    ...
    ```{r, echo=FALSE, out.width="100%"}
    knitr::include_graphics("figures/SC-additive.gif")
    ```
...
-   **Drift**: Used to apply a linear shift to an entire data file. This is sometimes needed when a sensor is swapped out and the data has a large jump due to drift in the calibration. To apply this kind of correction, ensure you're on the *Drift* option within the **Corrections** panel.
    ...
    ```{r, echo=FALSE, out.width="100%"}
    knitr::include_graphics("figures/temp-drift.gif")
    ```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/fdom-smoothing.gif")
```
...
4.  Toggle **View the data by period** and check that the interpolations look reasonable. **Make sure to untoggle this option after checking or when you apply edits it will only apply to the section you're viewing**.
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/turb-interp.gif")
```

### Step 5.1.5 Save Project.

1.  **Save the project.** Navigate back to the **Download Data** tab and click the **Export Project** button to save the edits to your project. When prompted if you want to overwrite your project you can safely overwrite it unless you want to keep a copy of your project prior to the current edits.
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/fdom-corrections.gif")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/data-export.gif")
```
